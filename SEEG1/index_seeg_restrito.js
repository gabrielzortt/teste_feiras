...
const EF = {
    // Transporte (Inventário Nacional de Emissões Atmosféricas por Veículos Automotores Rodoviários – IEMA)
    // Gasolina automotiva: fator de emissão de CO2 em kg/L (faixa 2004–2024 = 2,23 kgCO2/L)
    // (ver Tabela 4.1.1.1.1.c)
    transport: {
        gasoline_kg_per_L: 2.23,
        // Transporte público: fator por passageiro-km (kgCO2e/km). Ajuste se você tiver um fator local.
        public_kg_per_km: 0.089
    },

    // Energia elétrica (Inventário 2025): fator médio anual 2025 em tCO2/MWh,
    // que é numericamente igual a kgCO2/kWh.
    electricity_kg_per_kwh: 0.0461
};
const LAND = {"beef":0.15213,"chicken":0.00064,"pork":0.00521,"milk":0.0017,"eggs":6e-05,"rice":0.00149,"beans":0.00091};
...
// UI: alternar perguntas de transporte (privado x público)
const transportModeEl = document.getElementById('transportMode');
const privateTransportFields = document.getElementById('privateTransportFields');
const publicTransportFields = document.getElementById('publicTransportFields');

function updateTransportUI() {
    const mode = transportModeEl?.value || 'privado';
    if (privateTransportFields) privateTransportFields.style.display = (mode === 'privado') ? 'block' : 'none';
    if (publicTransportFields) publicTransportFields.style.display = (mode === 'publico') ? 'block' : 'none';
}

if (transportModeEl) {
    transportModeEl.addEventListener('change', updateTransportUI);
    updateTransportUI();
}
...
if (calculator) {
    calculator.addEventListener('submit', (e) => {
        ...
        const transport = calculateTransport(data); // tCO2e/ano
        const energy = calculateEnergy(data);       // tCO2e/ano
        const food = calculateFood(data);           // tCO2e/ano
        ...
        updateCategoryChart(transport, energy, food, waste);
        updateComparisonChart(finalEmissions);
        generateRecommendations({...data, transport, energy, food, waste}, finalEmissions);
...
function collectFormData() {
    const transportMode = document.getElementById('transportMode')?.value || 'privado';
    ...
    return {
        ...
        // Transporte
        transportMode,
        gasoline: clampNumber(document.getElementById('gasoline')?.value, 0),          // L/mês (apenas veículo privado)
        publicKm: clampNumber(document.getElementById('publicKm')?.value, 0),          // km/mês (apenas transporte público)

        // Energia (inventário 2025)
        electricity: clampNumber(document.getElementById('electricity')?.value, 0),    // kWh/mês

        // Alimentação (SEEG: somente alimentos)
        ...
        // Resíduos (SEEG: somente resíduos)
...
function calculateTransport(data) {
    // Resultado em tCO2e/ano
    if (data.transportMode === 'publico') {
        const kg = 12 * data.publicKm * EF.transport.public_kg_per_km;
        return kg / 1000;
    }

    // Veículo privado: somente gasolina (L/mês)
    const kg = 12 * data.gasoline * EF.transport.gasoline_kg_per_L;
    return kg / 1000;
}

// ====== 5.2 ENERGIA (Inventário 2025) ======
function calculateEnergy(data) {
    // Resultado em tCO2e/ano
    const kg = 12 * data.electricity * EF.electricity_kg_per_kwh;
    return kg / 1000;
}

// ====== 5.3 ALIMENTAÇÃO (SEEG – somente alimentos) ======
function calculateFood(data) {
    ...
    return beef + chicken + pork + milk + eggs + rice + beans;
}

// ====== 5.2 USO DA TERRA (itens 2.1 a 2.8) ======

// ====== 5.3 RESÍDUOS (itens 3.1 a 3.3.3, replicando a planilha) ======
// ====== 5.4 RESÍDUOS (SEEG – somente resíduos) ======
function calculateWaste(data) {
    const uf = data.state;
    const wf = getWasteFactor(uf);

    // Fatores estaduais per capita (tCO2e/ano)
    const effDom = wf.eff_dom;
    const effInd = wf.eff_ind;

    // Resíduos sólidos (sem reduções)
    const solidNoReductions = wf.solid;

    // Abatimentos (per capita)
    const abatRecycling = ABATEMENTS.recycling;
    const abatComposting = ABATEMENTS.composting;

    // Resíduos sólidos (com reduções)
...
// 7) Gráficos
// 7) Gráficos
function updateCategoryChart(transport, energy, food, waste) {
    if (!categoryChart) return;
    categoryChart.innerHTML = '';

    const categories = [
        { name: 'Transportes', value: transport },
...
// 8) Recomendações (simples e coerentes com as entradas)
function generateRecommendations(data, totalEmissions) {
    ...
    // Transporte
    if (data.transportMode === 'privado' && data.gasoline > 0) {
        recs.push({
            icon: '🚗',
            title: 'Reduzir consumo de gasolina',
            description: 'Avalie caronas, transporte coletivo e manutenção do veículo (pneus calibrados, revisões).',
            impact: 'Redução proporcional aos litros/mês',
            priority: 'Alta'
        });
    } else if (data.transportMode === 'publico' && data.publicKm > 0) {
        recs.push({
            icon: '🚌',
            title: 'Otimizar deslocamentos',
            description: 'Agrupe trajetos, reduza viagens desnecessárias e considere modos ativos (caminhada/bicicleta) quando seguro.',
            impact: 'Redução proporcional aos km/mês',
            priority: 'Média'
        });
    }
    ...
    if (data.beef > 0) {
        recs.push({
            ...
            description: 'A carne bovina costuma ter alta intensidade de emissões na alimentação.',
    ...
    if (data.recycling !== 'Sim') {
        recs.push({
            icon: '♻️',
            title: 'Implementar reciclagem',
            description: 'Separar e destinar corretamente recicláveis reduz emissões no modelo.',
